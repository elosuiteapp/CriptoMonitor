-- 120: Robô 2.0 — PESO POR BLOCO + decisão por FORÇA PONDERADA (10/jul, pedido do dono).
-- A decisão do conf2 deixa de ser contagem (3-de-5) e vira Σ(peso × força do bloco), −100..+100,
-- com limiar de ENTRADA (conf2_enter) + HISTERESE (conf2_hold; segura com força menor, sai perto de 0).
-- Pesos ajustáveis no painel; default = 3 fortes (Estrutura 30 · Micro 25 · Técnico 20) + 2 leves
-- (Fluxo 13 · Posicionamento 12). Também expõe conf2_stop_atr (largura ×ATR do stop de catástrofe,
-- antes hardcoded no bot-run). LIÇÃO sql/111: coluna nova em bot_config = atualizar a RPC junto (whitelist).

alter table public.bot_config
  add column if not exists conf2_weights  jsonb   not null default '{"estrutura":30,"micro":25,"tecnico":20,"fluxo":13,"posicionamento":12}'::jsonb,
  add column if not exists conf2_enter    numeric not null default 30,
  add column if not exists conf2_hold     numeric not null default 10,
  add column if not exists conf2_stop_atr numeric not null default 4;

-- RPC v5: whitelist de sql/118 (v4) + conf2_weights/enter/hold/stop_atr.
...
begin
  ...
  update public.bot_config set
    ...
    -- Robô 2.0 força ponderada (sql/120):
