-- 113_bot_zone_discipline.sql — DISCIPLINA DE ZONA (spec do dono 06/jul: "no topo vermelho não
-- compra — espera venda ou rompimento forte; no fundo verde, espelho"). Fases L/M/O do backtest:
-- a variante M2 (rompimento de swing ≤16 velas OU estrutura interna já virada) foi a ÚNICA a
-- melhorar as 4 moedas — BTC +0,29→+0,36 · ETH +0,08→+0,11 · SOL +0,64→+0,72 · BNB +0,21→+0,22;
-- R total recorde +59,9 (vs +56,1 do v24). A versão composta estrita (volume 1,5×+ADX) tem R
-- médio maior mas quebra o ETH — fica como knob no backtester (zone_mode='strong').
alter table public.bot_config add column if not exists zone_discipline boolean not null default true;

-- bot_set_config: aceitar o campo (regra: coluna nova = RPC junto).
-- (recria com TODOS os campos vivos; única mudança vs sql/112 é a linha zone_discipline)
