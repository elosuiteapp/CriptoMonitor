...
import BotChart, { type BotCandle, type BotMarker, type BotPriceLine } from "../BotChart";
import Card from "../../ui/Card";
import { BARS } from "../../../lib/bot/constants";
import type { Config } from "../../../lib/bot/types";

/** Gráfico com marcações — seletor de moeda/TF + <BotChart>. Só PREÇO + pontos de entrada/saída
 *  e linhas de operação (entrada/stop/pico). Sem sub-painel de indicadores (removido a pedido do dono). */
export default function BotChartSection({ selInst, cfg, setCfg, ASSET_LIST, selAsset, setSelAsset, refresh, busy, connected, candles, markers, priceLines, dec }: {
  selInst: string;
  cfg: Config | null;
  ...
  return (
      <Card className="hover:border-foreground/15 hover:shadow-card-hover p-4">
        ...
        {connected && candles.length > 0 ? (
          <BotChart candles={candles} markers={markers} priceLines={priceLines} lines={[]} decimals={dec} height={420} fitKey={`${selInst}-${cfg?.bar ?? ""}`} />
        ) : (
