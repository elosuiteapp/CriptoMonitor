...
const UPGRADE: { slug: "pro" | "expert"; tag?: string; features: string[] }[] = [
  { slug: "pro", tag: "Mais popular", features: ["20 ativos · dados a cada 5 min", "Gamma, volatilidade e camadas no gráfico", "Alertas e relatórios diários por IA"] },
  { slug: "expert", features: ["Tudo do Pro", "Smart Money & On-chain · 100 moedas", "Arquivo completo · 30 análises de IA/dia"] },
];
...
export default function AccountDrawer({ user, welcome, intentPlan, onClose }: Props) {
  ...
  async function saveProfile(e: React.FormEvent) {
    ...
    setProfileMsg(error ? "Não foi possível salvar." : "Perfil atualizado ✓");
  }
  ...
  async function startCheckout(slug: "pro" | "expert") {
    ...
    try {
      ...
      if (data?.code === "cpf_required") {
        setCheckoutMsg("Informe seu CPF no perfil abaixo e tente de novo.");
        return;
      }
      if (data?.url) {
      ...
      setCheckoutMsg(e instanceof Error ? e.message : "Falha ao iniciar o checkout.");
    } finally {
  ...
  async function doDowngrade() {
    ...
    setCheckoutBusy(null);
    setConfirming(false);
    if (error) return setCancelMsg(error instanceof Error ? error.message : "Falha ao cancelar.");
    const d = data as { code?: string; message?: string } | null;
    setCancelMsg(d?.message ?? "Você volta ao Free ao fim do período já pago — o acesso fica garantido até lá.");
  }

  const statusLine = canceling
    ? `Não renova. Acesso até ${fmtDate(periodEnd)}.`
    : status === "past_due"
      ? "Pagamento em atraso — regularize para manter o acesso."
      : isPaid
        ? `Renova automaticamente em ${fmtDate(periodEnd)}.`
    ...
    <div className="fixed inset-0 z-[100]">
      ...
      >
        <div className="flex items-start justify-between gap-3 border-b border-border px-5 py-3.5">
          <div>
            <h2 className="text-lg font-bold text-foreground">{welcome ? "Bem-vindo ao OrbeView 🎉" : "Sua conta"}</h2>
            <p className="text-xs text-muted-foreground">
              {welcome ? "Finalize seu cadastro e escolha seu plano." : "Plano, assinatura e dados do perfil."}
            </p>
          </div>
          <button onClick={close} aria-label="Fechar (ESC)" className="shrink-0 text-lg leading-none text-muted-foreground transition-colors hover:text-foreground">
            ✕
          </button>
        </div>
        ...
        <div className="flex-1 space-y-6 overflow-y-auto px-5 py-5">
          ...
          <section>
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Plano</span>
              <span
            ...
            {!isPaid ? (
              <>
                  ...
                  {UPGRADE.map((u) => {
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-baseline justify-between gap-2">
                          ...
                          {m ? (
                            <div className="text-right">
                              ...
                              <span className="text-xs text-muted-foreground">{showAnnual ? "/ano" : "/mês"}</span>
                            </div>
                          ) : null}
                        </div>
                        ...
                        >
                          {checkoutBusy === u.slug ? "Redirecionando…" : `Assinar ${PLAN_NAME[u.slug]}`}
                        </button>
                      </div>
                    );
                  })}
                ...
                <p className="mt-2 text-center text-[11px] text-muted-foreground">Pix, boleto ou cartão (Asaas) · cancele quando quiser</p>
              </>
            ) : (
                  ...
                  (confirming ? (
                    <div className="rounded-lg border border-rose-200 bg-rose-50 p-3 dark:border-rose-500/20 dark:bg-rose-500/10">
                      <p className="text-xs text-rose-700 dark:text-rose-400">
                        Rebaixar para o <b>Free</b>? Acesso garantido até <b>{fmtDate(periodEnd)}</b>, sem nova cobrança.
                      </p>
                      <div className="mt-2 flex gap-2">
                        <button onClick={doDowngrade} disabled={checkoutBusy === "free"} className="rounded-lg bg-rose-600 px-3 py-1.5 text-xs font-semibold text-white hover:opacity-90 disabled:opacity-50">
                          {checkoutBusy === "free" ? "…" : "Sim, rebaixar"}
                        </button>
          ...
          <section className="border-t border-border pt-5">
            <span className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Perfil</span>
            <form onSubmit={saveProfile} className="mt-3 space-y-3">
              <label className="block">
                ...
                <input className={fieldCls} value={name} onChange={(e) => setName(e.target.value)} placeholder="Seu nome" />
              </label>
              <label className="block">
              ...
              <label className="block">
                ...
                <span className="mt-1 block text-[11px] text-muted-foreground">Necessário para pagamento em reais (Pix/cartão).</span>
              </label>
              <label className="block">
