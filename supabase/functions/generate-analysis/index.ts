...
Deno.serve(async (req) => {
  ...
  // A analise cripto e recurso do MODULO cripto: exige 'crypto' em plan.modules (planos
  // legados pro/expert ja tem modules={crypto}). Sem isso, assinante so-B3/so-Forex (que tem
  // BTC de vitrine + cota de IA do proprio modulo) gerava analise cripto direto no endpoint
  // — bypass de paywall (auditoria 02/jul, docs/crypto-launch-audit.md).
  const modules = (plan.modules as string[] | null) ?? [];
  if (!modules.includes("crypto")) {
    return json(403, { error: isEn ? "Crypto AI analysis is not included in your plan." : "A analise por IA do modulo cripto nao esta incluida no seu plano." });
  }
  ...
  // Noticias recentes — no idioma da analise (sem o filtro, manchete PT vazava em analise EN)
  const { data: news } = await admin
    .from("news_feed").select("title, source").contains("assets", [ativo]).eq("lang", lang)
