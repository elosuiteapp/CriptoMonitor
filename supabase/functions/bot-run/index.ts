...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const processAsset = async (asset: string, instId: string) => {
      ...
      // Histórico rolante dos blocos p/ o gráfico (indicador no tempo): 1 ponto/ciclo, compacto [t,wf,e,m,f,p,tec], cap 300.
      const bscore = (k: string) => conf2g.find((x) => x.key === k)?.score ?? 0;
      const blockHist = [...(Array.isArray(st.block_hist) ? st.block_hist : []), [Math.floor(Date.now() / 1000), conf2Wf, bscore("estrutura"), bscore("micro"), bscore("fluxo"), bscore("posicionamento"), bscore("tecnico")]].slice(-300);
      await saveReading(asset, { last_bias: bias, last_conviction: conviction, last_decision: decision, last_reading: reading, last_run: new Date().toISOString(), block_hist: blockHist });
