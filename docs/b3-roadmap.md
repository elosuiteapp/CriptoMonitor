...
- **Detalhe por FII (VP/Cota, nº de cotistas, patrimônio, último rendimento)** — **PENDENTE (pedido do dono, 25/jun)**. Hoje o cockpit do FII usa o `fii_resultado.php` em LOTE (DY, P/VP, FFO, cap rate, vacância, qtd imóveis, valor mercado, liquidez, segmento) + IFIX + DY×CDI + rendimento projetado + comparação por segmento. Falta o detalhe **por ativo**, que enriquece mais:
  - **Fonte 1 — Fundamentus `detalhes.php?papel={TICKER}`** (grátis, scrape HTML latin1, **1 request POR FII**): tem **VP/Cota** (valor patrimonial por cota), **Rendimento distribuído por cota** (último), **Patrimônio** e **Valor de mercado**. Confirmado ao vivo (MXRF11) que esses campos existem na página. Implementar como novo modo na edge `b3-data` (ex.: `{mode:"fii-detail", ticker}`) chamado ao selecionar o FII; parsear pelos rótulos. **Risco:** scrape por ativo, mais frágil que o lote (cai se o site mudar) — fallback p/ os dados do lote.
  - **Fonte 2 — CVM `dados.cvm.gov.br`** (grátis, CSV/zip): informe mensal de FII com **patrimônio líquido e nº de cotistas OFICIAIS**. Mais robusto que scrape, mas **pesado** (baixar/parsear CSVs grandes) e com **defasagem** (1–3 meses). Melhor como job server-side, não on-demand.
  - **Valor:** VP/Cota dá o P/VP mais preciso e o "deságio em R$"; nº de cotistas = popularidade/liquidez; patrimônio = porte do fundo. Ver [[b3-data-sources]].
