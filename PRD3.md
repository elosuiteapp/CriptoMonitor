...
**Versão:** 1.6 · **Data:** Junho/2026 · **Autor:** Marcos (Back Hub Soluções Digitais)
**Status:** Aprovado para desenvolvimento (MVP)

> **Changelog v1.6 (aditivo — não altera o MVP em curso):** Especificada a **Fase 6 — Cockpit Expandido** com 5 adições de leitura institucional: paredes do order book, fluxo de exchanges (in/outflow), DXY + correlações macro, delta de Open Interest e Volume Profile/Point of Control. Adicionadas duas novas fontes gratuitas (Yahoo Finance/stooq para macro, Blockchair/Etherscan para on-chain). Reestruturação do dashboard em 3 abas (§8.7) com gating por plano. Tiers 2 e 3 documentados no backlog pós-Fase 6. **Nenhum elemento do MVP v1.5 foi alterado.**
>
...
| 11 | **Yahoo Finance / stooq** (Fase 6) | DXY (Dollar Index), S&P 500, ouro, treasury yields | Macro & correlações com cripto | REST, sem chave |
| 12 | **Blockchair + Etherscan + Solscan** (Fase 6) | Saldos de exchanges, transações grandes | Exchange flows + whale alerts (Tier 2) | REST, free tier (sem chave para Blockchair; chaves gratuitas para Etherscan/Solscan) |

**Regra de arquitetura:** o coletor apenas coleta, formata e grava. Toda interpretação acontece na camada de IA.

**Escopo de fontes por fase:** as 10 primeiras compõem o **MVP (Fases 1-5)**. As fontes 11 e 12 entram na **Fase 6 — Cockpit Expandido** (§9), sem afetar o que já existe.
...
### 8.7 Estrutura em abas — escada de profundidade (v1.6)

**Motivação:** com a Fase 6 entrando, a página do ativo cresce em volume de informação. Para preservar o princípio do "10 segundos" (§8.3), o conteúdo passa a ser organizado em **3 abas dentro da página de cada ativo**, formando uma escada natural de profundidade que também serve de escada de planos.

| Aba | Conteúdo | Plano mínimo |
|---|---|---|
| **Cockpit Principal** | Tudo do MVP v1.5: gráfico com camadas, módulo Gamma, bloco "Fluxo, liquidez e sentimento" (§8.6.3), notícias | Free (com restrições do plano) / Pro (completo) |
| **Macro & Correlações** (Fase 6) | DXY, S&P 500, ouro, correlação 30d com o ativo, calendário econômico | Pro |
| **Smart Money & On-chain** (Fase 6) | Exchange flows, whale alerts, MVRV, cronograma de unlocks | Expert |

**Comportamento:** a aba "Cockpit Principal" é a default; o usuário Free vê as outras duas como abas bloqueadas com cadeado (mesma vitrine do §8.3). Trocar de aba **não** dispara nova coleta — apenas troca a visualização sobre dados já no Supabase. A análise da IA continua sendo uma só, integrando dados de todas as abas relevantes ao plano do usuário.

**Pré-condição:** §8.7 só entra junto com §8.8. Antes da Fase 6, a página do ativo segue sem abas, exatamente como o MVP v1.5 entrega.

### 8.8 Fase 6 — Cockpit Expandido (Tier 1 de adições institucionais, v1.6)

Cinco adições que aproximam o cockpit do que mesas profissionais usam. Implementação **posterior ao fechamento do MVP** (Fases 1-5). Nenhum item desta seção altera o MVP.

#### 8.8.1 Paredes do order book (camada nova no gráfico — Cockpit Principal)

**O que faz:** identifica e plota no gráfico os grandes lances *parados* nos books de Binance e Coinbase — linhas horizontais com a magnitude do book em dólares.

**Diferença em relação às liquidações:** liquidações mostram onde dinheiro vai *sair* à força; paredes do book mostram onde dinheiro está *posicionado* esperando ser tocado. Juntos completam a foto de "ímãs magnéticos" do preço.

**Fonte:** Binance e Coinbase WebSocket (`@depth` ou `level2`). Filtro: lances acima de US$ 1M (BTC), US$ 500k (ETH), US$ 100k (SOL). Refresh a cada 30s no frontend via canal Realtime.

**Tabela:** `orderbook_walls` (asset, exchange, side, price, notional_usd, ts).

#### 8.8.2 Fluxo de exchanges (in/outflow) — card na aba Smart Money

**O que faz:** calcula o saldo líquido de tokens entrando e saindo dos endereços conhecidos de exchanges nas últimas 24h e 7d.

**Tradução no card:** "Saíram 3.200 BTC das exchanges nas últimas 24h — sinal de acumulação" (semáforo verde) ou "Entraram 12.500 ETH nas exchanges — possível pressão vendedora" (vermelho).

**Fonte:** Blockchair API (gratuita, sem chave) + lista pública curada de endereços de exchanges (Etherscan tags). Coleta a cada 30 min (movimento on-chain é mais lento que o de mercado).

**Tabela:** `exchange_flows` (asset, exchange, inflow_24h, outflow_24h, netflow_24h, netflow_7d, ts).

#### 8.8.3 DXY + correlações macro — aba Macro & Correlações

**O que faz:** apresenta o Dollar Index (DXY), S&P 500 e ouro com sua correlação 30 dias contra o ativo selecionado, mais um calendário das próximas decisões macro relevantes (FOMC, CPI, PPI).

**Tradução nos cards:** "DXY +1.2% em 7d e correlação −0.68 com BTC — vento contra de força moderada" / "Próximo FOMC em 18/06: volatilidade típica de ±3% em 24h".

**Fonte:** Yahoo Finance (endpoint não-oficial gratuito) ou stooq.com para preços; Trading Economics ou scraping de calendário econômico para datas. Coleta a cada 30 min.

**Tabela:** `macro_assets` (symbol, price, change_24h, change_7d, ts) + `economic_calendar` (event, country, date_utc, importance).

#### 8.8.4 Delta de Open Interest — card no Cockpit Principal

**O que faz:** já temos OI absoluto via Coinalyze; o delta entrega leitura de fluxo: "OI cresceu +8% em 4h enquanto preço caiu 2% — novo dinheiro entrou vendido". A combinação delta-de-OI × movimento-de-preço é leitura clássica de fluxo institucional.

**Implementação:** view SQL `v_oi_delta` que computa variação percentual do OI em janelas de 1h, 4h e 24h a partir da tabela `derivatives` existente. **Zero fonte nova.**

**Tradução automática conforme combinação de sinais:**
- OI ↑ + preço ↑: "Novas compras alavancadas — momentum"
- OI ↑ + preço ↓: "Novas vendas alavancadas — atenção a squeeze"
- OI ↓ + preço ↑: "Short cobrindo — squeeze em andamento"
- OI ↓ + preço ↓: "Long capitulando — desalavancagem"

#### 8.8.5 Volume Profile / Point of Control — camada no gráfico (Cockpit Principal)

**O que faz:** calcula a distribuição de volume por nível de preço nos últimos N dias (padrão: 30) e identifica o **Point of Control (POC)** — o preço onde mais volume foi negociado. Plota como linha horizontal destacada no gráfico, além de um histograma lateral opcional com a distribuição completa.

**Por que importa:** o POC funciona como ímã de preço e como referência institucional de "value area". Combinado com Call/Put Walls e bolsões de liquidações, completa a tríade de níveis magnéticos.

**Implementação:** calculado a partir dos próprios candles que já temos. Pode rodar como view SQL ou job leve no aggregator. **Zero fonte nova.**

**Tabela:** `volume_profile` (asset, window_days, poc_price, value_area_high, value_area_low, ts).
...
### Fase 6 — Cockpit Expandido (v1.6, semana 9-11) — pós-MVP

Inicia somente após a Fase 5 fechada e MVP em produção com primeiros usuários reais. Nenhum item desta fase altera o MVP.

17. Reestruturação da página do ativo em **3 abas** conforme §8.7.
18. Novas fontes 11 e 12 (Yahoo Finance/stooq, Blockchair/Etherscan/Solscan) com módulos isolados em `collector/sources/`.
19. Implementação dos 5 itens do §8.8: paredes do order book, exchange flows, DXY + correlações, delta de OI, Volume Profile/POC.
20. Atualização da camada de IA para considerar as novas métricas no prompt de análise.
21. Atualização do gating: Macro & Correlações libera no Pro; Smart Money & On-chain libera no Expert.

### Pós-Fase 6 (backlog — Tier 2 e além)

**Tier 2 — adições profissionais:**
- **MVRV Ratio** — métrica de topos e fundos de BTC.
- **Whale alerts** — transações > US$ 1M em tempo real (WhaleAlert API).
- **Calendário econômico no eixo do gráfico** — FOMC, CPI marcados como eventos.
- **Put/Call Ratio agregado** — sentimento do mercado de opções (dados já existentes na Deribit).
- **Cronograma de unlocks de tokens** — vestings e desbloqueios futuros (CryptoRank API).

**Tier 3 — sofisticado, só com base de usuários grande:**
- Smart money wallet tracking (rastrear carteiras institucionais conhecidas).
- Footprint chart / order flow visual avançado.
- IV term structure das opções na Deribit.
- Funding rate divergence entre exchanges (arbitragem).

**Outras evoluções de produto:**
...
- Replay histórico do cockpit (rebobinar o mercado em qualquer data).
...
| Yahoo Finance API não-oficial (Fase 6) pode mudar/quebrar | Falha na coleta macro | Fallback para stooq.com (free, mais estável); coleta a cada 30 min reduz exposição |
| Blockchair / Etherscan free tier (Fase 6) pode ser limitado para coleta on-chain frequente | Atrasos no card de exchange flows | Coleta a cada 30 min (não 5 min) — on-chain é naturalmente mais lento; cachear endereços de exchanges em tabela local |
| Carga de WebSocket de book depth (Fase 6) pode pesar | Latência ou desconexões | Throttle de 30s no frontend; agregar antes de gravar; só capturar lances acima do filtro de notional |
...
15. **Expansão aditiva via Fase 6 (v1.6)** — adições institucionais (paredes do book, exchange flows, macro, OI delta, Volume Profile) entram em fase pós-MVP sem alterar o que já existe. Princípio: o MVP fecha primeiro, expansão depois.
16. **Estrutura em abas como escada de planos (v1.6)** — Cockpit Principal (Free/Pro) → Macro & Correlações (Pro) → Smart Money & On-chain (Expert). Resolve o problema de saturar o "10 segundos" do §8.3 e cria mais uma alavanca de monetização sem encarecer.
